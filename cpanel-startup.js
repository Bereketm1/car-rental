...
console.log('🚀 Zelalem Motors: Bootstrapping for cPanel (JS Mode)...');
...
console.log('📦 Starting microservices (JS)...');
services.forEach(s => {
...
if (gatewayPath && fs.existsSync(gatewayPath)) {
  ...
  console.error('❌ Gateway index.js NOT FOUND. Please ensure it was pushed to GitHub.');
}

// 3. Seed after 15 seconds
setTimeout(() => {
  const seedPath = findCorrectPath(__dirname, 'seed.js');
  if (seedPath) {
    console.log('🌱 Starting background seed...');
    spawn('node', [seedPath], { stdio: 'inherit', detached: true }).unref();
  }
}, 15000);
