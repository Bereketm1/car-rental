...
export default function Analytics() {
  ...
  const financingPieData = useMemo(
    () => ({
      ...
      datasets: [{ data: financingBreakdown.map((item) => item.value), backgroundColor: financePalette, borderWidth: 0 }],
    }),
    [financingBreakdown],
  );

  const salesByMake = useMemo(() => safeArray(data.sales.byMake), [data.sales.byMake]);
  const topPartners = useMemo(() => safeArray(data.partners.topPartners), [data.partners.topPartners]);

  if (loading) {
    return <div className="page-shell"><div className="empty-state"><h3>Loading analytics dashboard...</h3></div></div>;
  }
    ...
    <div className="page-shell">
      <div className="page-header row-between">
        <div>
          ...
          <p>Management view of vehicle sales, financing outcomes, partner contribution, and marketplace revenue.</p>
        </div>
      ...
      <div className="stats-grid">
        <MetricCard icon={BadgeDollarSign} label="Revenue" value={formatCurrency(data.revenue.totalRevenue, { compact: true, maximumFractionDigits: 1 })} detail="Total transaction revenue" tone="accent" />
        <MetricCard icon={CarFront} label="Vehicles sold" value={data.summary.totalVehiclesSold || 0} detail="Sales volume" tone="success" />
        ...
        <MetricCard icon={Activity} label="Active deals" value={data.summary.totalActiveDeals || 0} detail={`${data.summary.totalLeads || 0} leads in funnel`} tone="info" />
      </div>
        ...
        <div className="card chart-card">
          <div className="card-header compact">
            <div>
              ...
              <div className="card-subtitle">Chart.js trend for revenue and commissions by month.</div>
            </div>
          </div>
        ...
        <div className="card chart-card">
          <div className="card-header compact">
            <div>
              ...
              <div className="card-subtitle">Chart.js distribution of approvals, pending files, and rejections.</div>
            </div>
          </div>
          <div className="chart-shell">
            <Doughnut
          ...
          <div className="pill-list">
            {financingBreakdown.map((item, index) => (
              <span key={item.label} className="pill" style={{ color: financePalette[index % financePalette.length] }}>
        ...
        <div className="card chart-card">
          <div className="card-header compact">
            <div>
              ...
              <div className="card-subtitle">Recharts bar chart of sold units by manufacturer.</div>
            </div>
          </div>
          <div className="chart-shell">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={salesByMake} margin={{ top: 8, right: 8, left: 0, bottom: 8 }}>
        ...
        <div className="card">
          <div className="card-header compact">
            <div>
              ...
              <div className="card-subtitle">Top partners by deal throughput and commissions.</div>
            </div>
          </div>
          <div className="list-stack">
            {topPartners.map((partner) => (
              <div key={partner.name} className="list-row">
                ...
                <div className="list-row-meta">Commission {formatCurrency(partner.commission || 0, { compact: true, maximumFractionDigits: 1 })} • Rating {partner.rating || 0}</div>
              </div>
            ))}
            {!topPartners.length ? <div className="empty-state compact"><h3>No partner analytics yet</h3><p>Partner statistics will appear when analytics data is available.</p></div> : null}
          </div>
        </div>
      </div>
      ...
      <div className="card">
        <div className="card-header compact">
          <div>
            ...
            <div className="card-subtitle">Indicators aligned with marketplace maturity objectives.</div>
          </div>
        </div>
        <div className="list-stack">
          ...
          <div className="list-row">
            ...
            <p>{data.summary.totalPartners || 0} active partners participating in supply and financing.</p>
          </div>
          <div className="list-row">
            <div className="list-row-title">Conversion efficiency</div>
            <p>{formatPercent(data.summary.conversionRate || 0)} lead-to-sale conversion in the current analytics model.</p>
          </div>
        </div>
      </div>
    </div>
