...
let SearchController = class SearchController {
    ...
    async globalSearch(q) {
        ...
        const customers = this.unwrapCollection(custRes).filter((c) => (c.firstName || '').toLowerCase().includes(query) ||
            (c.lastName || '').toLowerCase().includes(query) ||
            (c.email || '').toLowerCase().includes(query) ||
            (c.phone || '').includes(query));
        const vehicles = this.unwrapCollection(vehRes).filter((v) => (v.make || '').toLowerCase().includes(query) ||
            (v.model || '').toLowerCase().includes(query) ||
            (v.condition || '').toLowerCase().includes(query) ||
            (v.supplierName || '').toLowerCase().includes(query));
        const deals = this.unwrapCollection(dealRes).filter((d) => (d.customerName || '').toLowerCase().includes(query) ||
